...
export class UsersService {
    ...
    async getUsersList(query: GetUsersListDto) {
        ...
        const formattedUsers = users.map(user => {
            const { password, user_roles, ...userWithoutPassword } = user;
    ...
    async getUserById(id: number) {
        const user = await this.prisma.user.findUnique({ where: { id }, include: { user_roles: { include: { role: true } } }, });
        if (!user)
    ...
    async updateUserRoles(dto: UpdateRolesDto) {
        ...
        const user = await this.prisma.user.findUnique({ where: { id: dto.id }, include: { user_roles: { include: { role: true } } }, });
        if (!user) {
    ...
    async resetPassword(id: number) {
        ...
        const user = await this.prisma.user.findUnique({ where: { id } });
        if (!user) throw new HttpException("Сотрудник не найден", HttpStatus.NOT_FOUND);
