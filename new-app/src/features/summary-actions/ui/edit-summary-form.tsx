...
export function EditSummaryForm({ data }: { data: SummaryEntity }) {
  ...
  return (
    <FormLayout
      title="Редактирование сводки" description="Измените данные и сохраните изменения"
      onClose={handleClose} footer={undefined}
    >
      <form id="form-rhf-demo" onSubmit={form.handleSubmit(onSubmit)}>
        <p>Здесь будет форма редактирования сводки с id = {editId}, когда мы придумаем, что редактировать</p>
        <p>{data.product.code} {data.product.marking} {data.batch.name}</p>
      </form>
    </FormLayout>
  )
