...
export default function ConveyorCombobox({
  items,
}: {
  items: ConveyorEntity[];
}) {
  ...
  const selectedIds = params.conveyors ?? [];
  ...
  const onSelect = (id: string) => {
    ...
    const nextConveyors = Array.from(current);
    setParams({
      conveyors: nextConveyors.length > 0 ? nextConveyors : null,
      page: 1,
    });
  };
            ...
            {selectedIds.length > 0 && (
              <>
                ...
                <CommandGroup>
                  <CommandItem
                    onSelect={() => {
                      setParams({ conveyors: null, page: 1 });
                    }}
