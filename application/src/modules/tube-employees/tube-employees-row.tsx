...
export default function TubeEmployeesRow({ row }: { row: ITubeEmployee }) {
  ...
  const openEditModal = useTubeEmployeesEditModalStore(
    useShallow((state) => state.setOpen),
  );
  const setId = useTubeEmployeesEditModalStore(
    useShallow((state) => state.setId),
  );
  const setName = useTubeEmployeesEditModalStore(
    useShallow((state) => state.setName),
  );
  const setBarcode = useTubeEmployeesEditModalStore(
    useShallow((state) => state.setBarcode),
  );
  const setRank = useTubeEmployeesEditModalStore(
  ...
  return (
    <tr key={row.id}>
      ...
      <td style={{ width: 64, textAlign: "center", padding: "12px 6px" }}>
        <StyledTypography
          text={row.banned ? "Запрещен" : "Разрешен"}
