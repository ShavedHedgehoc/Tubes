...
import FilterSwitchButton, {
  FilterSwitchButtonProps,
} from "../../shared/ui/filter-switch-button";

export default function DashFilterSwitcher() {
  const smallCardView = useDashFilterStore(
    useShallow((state) => state.smallCardView),
  );
  const setSmallCardView = useDashFilterStore(
    useShallow((state) => state.setSmallCardView),
  );
