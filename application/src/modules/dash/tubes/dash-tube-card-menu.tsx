...
export default function DashTubeCardMenu({
  conveyor,
}: {
  conveyor: IConveyorData;
}) {
  const setOpenFinish = useDashFinishModalStore(
    useShallow((state) => state.setOpen),
  );
  const setOpenStart = useDashSelectSummaryModalStore(
    useShallow((state) => state.setOpen),
  );
  const setTitleStart = useDashSelectSummaryModalStore(
    useShallow((state) => state.setTitle),
  );
  const setTitleFinish = useDashFinishModalStore(
    useShallow((state) => state.setTitle),
  );
  ...
  const setConveyorId = useDashSelectSummaryModalStore(
    useShallow((state) => state.setConveyorId),
  );
  ...
  return (
    <Dropdown>
      ...
      <Menu placement="bottom-end">
        <MenuItem
          disabled={disabledStartCondition}
