...
export default function TubeTresholdsFilterEmptySelector() {
  ...
  const selectedEmptyOption = useTubeTresholdsFilterStore(
    useShallow((state) => state.selectedEmptyOption),
  );
  const setSelectedEmptyOptions = useTubeTresholdsFilterStore(
    useShallow((state) => state.setSelectedEmptyOption),
  );
