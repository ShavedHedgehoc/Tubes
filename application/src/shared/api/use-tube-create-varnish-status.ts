...
export function useTubeCreateVarnishStatus() {
  const client = useQueryClient();

  const { mutate: createTubeVarnishStatus, isPending } = useMutation({
    mutationFn: TubeStatusesService.createVarnishStatus,
    onSuccess: () => {
  ...
  return { createTubeVarnishStatus, isPending };
}
