...
export default function SealantOperationsContent({
  summaryData,
}: {
  summaryData: ISummary | null;
}) {
  const { items, setSelectedOperation, employee, selectedOperation } =
    useSealantOperationsContent({
  ...
  if (!summaryData.sealantOperations.length)
    ...
    idle: (
      <VStack gap={12} h="full" justify="center">
        ...
        <Heading fontSize="3xl" color="fg.subtle">
          ...
          {summaryData.sealantStatus.operation_description}
        </Heading>
        <Heading fontSize="3xl" color="fg.subtle">
          Время начала:{" "}
          {formatTimeToString(summaryData.sealantStatus.createdAt)}
        </Heading>
      </VStack>
    ),
    working: (
