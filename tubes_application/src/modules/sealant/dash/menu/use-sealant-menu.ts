...
export default function useSealantMenu() {
  ...
  const handleOpenParametersClick = () => {
    ...
    setData({ key: SealantInputParams.CAP_MACHINE_SPEED, value: capMachineSpeed })
    setData({ key: SealantInputParams.TOTAL_AIR_PRESSURE, value: totalAirPressure })
    setData({ key: SealantInputParams.HOLDERS_FORWARD, value: holdersForward })
    setData({ key: SealantInputParams.HOLDERS_OPENING_LEFT, value: holdersOpeningLeft })
    setData({ key: SealantInputParams.HOLDERS_OPENING_RIGHT, value: holdersOpeningRight })
    setData({ key: SealantInputParams.HOLDERS_CLOSING, value: holdersClosing })
    setData({ key: SealantInputParams.INJECTION_TUBE_ORIENTATION_START, value: injectionTubeOrientationStart })
    setData({ key: SealantInputParams.INJECTION_TUBE_ORIENTATION_END, value: injectionTubeOrientationEnd })
