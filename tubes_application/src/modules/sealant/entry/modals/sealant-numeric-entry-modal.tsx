...
export default function SealantNumericEntryModal() {
  const { key, title, open, setOpen, minValue, maxValue, unit } =
    useSealantNumericEntryModalStore(
      useShallow((state) => ({
        key: state.key,
        title: state.title,
        open: state.open,
  ...
  if (!key) return null;
  return (
    <NumericEntryModal<SealantInputParams>
