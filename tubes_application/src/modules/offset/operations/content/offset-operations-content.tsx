  ...
  const content = {
    idle: (
      <VStack gap={12} h="full" justify="center">
        ...
        <Heading fontSize="3xl" color="fg.subtle">
          Выполняемая операция: {summaryData.offsetStatus.operation_description}
        </Heading>
        <Heading fontSize="3xl" color="fg.subtle">
          Время начала: {formatTimeToString(summaryData.offsetStatus.createdAt)}
        </Heading>
      </VStack>
    ),
    working: (
    ...
    finished: <></>,
  }[summaryData.offsetStatus.state];
