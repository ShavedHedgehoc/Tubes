...
export function useCreateExtrusionStatus() {
  const client = useQueryClient();

  const { mutate: createExtrusionStatus, isPending } = useMutation({
    mutationFn: StatusService.createExtrusionStatus,
    onSuccess: () => {
  ...
  return { createExtrusionStatus, isPending };
}
