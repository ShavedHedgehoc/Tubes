...
export function useCreateVarnishEntry() {
  const client = useQueryClient();

  const { mutate: createVarnishEntry, isPending } = useMutation({
    mutationFn: ParamsService.createVarnishEntry,
    onSuccess: () => {
  ...
  return { createVarnishEntry, isPending };
}
