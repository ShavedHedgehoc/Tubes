...
export function useCreateVarnishStatus() {
  const client = useQueryClient();

  const { mutate: createVarnishStatus, isPending } = useMutation({
    mutationFn: StatusService.createVarnishStatus,
    onSuccess: () => {
  ...
  return { createVarnishStatus, isPending };
}
