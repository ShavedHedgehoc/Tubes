...
export default function useVarnishMenu() {
  ...
  const handleOpenParametersClick = () => {
    ...
    const nozzleRegulatorAirPressure = String(params?.nozzle_regulator_air_pressure ?? "0");
    const cellsSpeed = String(params?.cells_speed ?? "0");
    const injectionAEndPosition = String(params?.injection_a_end_position ?? "0");
    const injectionBEndPosition = String(params?.injection_b_end_position ?? "0");
    const injectionCEndPosition = String(params?.injection_c_end_position ?? "0");
    const injectionDEndPosition = String(params?.injection_d_end_position ?? "0");
    const tubeMoldingStartPosition = String(params?.tube_molding_start_position ?? "0");
    const tubeMoldingEndPosition = String(params?.tube_molding_end_position ?? "0");


    setData({ key: VarnishInputParams.VARNISH_MACHINE_SPEED, value: varnishMachineSpeed })
    setData({ key: VarnishInputParams.TOTAL_AIR_PRESSURE, value: totalAirPressure })
    setData({ key: VarnishInputParams.FEED_CAN_AIR_PRESSURE, value: feedCanAirPressure })
    setData({ key: VarnishInputParams.NOZZLE_REGULATOR_AIR_PRESSURE, value: nozzleRegulatorAirPressure })
    setData({ key: VarnishInputParams.CELLS_SPEED, value: cellsSpeed })
    setData({ key: VarnishInputParams.INJECTION_A_END_POSITION, value: injectionAEndPosition })
    setData({ key: VarnishInputParams.INJECTION_B_END_POSITION, value: injectionBEndPosition })
    setData({ key: VarnishInputParams.INJECTION_C_END_POSITION, value: injectionCEndPosition })
    setData({ key: VarnishInputParams.INJECTION_D_END_POSITION, value: injectionDEndPosition })
    setData({ key: VarnishInputParams.TUBE_MOLDING_START_POSITION, value: tubeMoldingStartPosition })
    setData({ key: VarnishInputParams.TUBE_MOLDING_END_POSITION, value: tubeMoldingEndPosition })
